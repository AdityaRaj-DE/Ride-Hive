...
export default function DriverMap({
  ...
  return (
    <div className="absolute inset-0 z-0 group overflow-hidden bg-background">
      {/* Map Atmosphere Overlays */}
      <div className="absolute inset-x-0 bottom-0 h-64 bg-gradient-to-t from-background via-background/40 to-transparent z-10 pointer-events-none"></div>
      <div className="absolute inset-0 bg-accent/[0.02] dark:bg-emerald-500/[0.02] pointer-events-none z-10"></div>
      ...
      <div className="absolute top-12 left-1/2 -translate-x-1/2 z-20 w-fit">
         <div className="glass-card px-8 sm:px-10 py-4 sm:py-5 rounded-3xl sm:rounded-[2.5rem] flex items-center gap-6 sm:gap-8 border border-border shadow-2xl backdrop-blur-3xl group/hud hover:border-accent transition-all duration-700">
            <div className="relative">
               <div className="w-12 h-12 bg-accent/10 rounded-2xl flex items-center justify-center border border-accent/20 group-hover/hud:rotate-12 transition-all">
                  <Activity className="w-6 h-6 text-accent shadow-[0_0_15px_rgba(59,130,246,1)] animate-pulse" />
               </div>
            </div>
            
            <div className="flex flex-col">
               <div className="flex items-center gap-3">
                  <span className="text-[10px] font-black uppercase tracking-[0.4em] sm:tracking-[0.6em] text-accent italic">Grid Telemetry Active</span>
                  <div className="w-2 h-2 rounded-full bg-accent animate-pulse"></div>
               </div>
               <span className="text-[9px] sm:text-xs font-black uppercase tracking-widest text-muted mt-1 italic group-hover/hud:opacity-100 transition-opacity">AES-256 Quantum Handshake Secure</span>
            </div>
            
            <div className="border-l border-border pl-6 sm:pl-8 ml-2 flex items-center gap-3 sm:gap-4">
               <Cpu className="w-4 h-4 sm:w-5 sm:h-5 text-accent/40 group-hover/hud:text-accent transition-colors" />
               <ShieldCheck className="w-4 h-4 sm:w-5 sm:h-5 text-accent/40 group-hover/hud:text-accent transition-colors" />
            </div>
         </div>
      </div>
      
      {/* Scanning HUD Decorative Overlays */}
      <div className="absolute top-0 left-0 w-full h-full border-[20px] sm:border-[30px] border-accent/[0.02] pointer-events-none z-10 rounded-[2rem] sm:rounded-[4rem]"></div>
      <div className="absolute bottom-10 sm:bottom-20 left-10 sm:left-20 w-24 h-24 sm:w-40 sm:h-40 border-l border-b border-accent/20 pointer-events-none z-10 opacity-40 group-hover:scale-110 transition-transform duration-1000"></div>
      <div className="absolute top-10 sm:top-20 right-10 sm:right-20 w-24 h-24 sm:w-40 sm:h-40 border-r border-t border-accent/20 pointer-events-none z-10 opacity-40 group-hover:scale-110 transition-transform duration-1000"></div>
    </div>
  );
}
