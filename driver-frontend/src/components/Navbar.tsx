...
import { Home, Wallet, User, History, LayoutGrid } from 'lucide-react';
...
const Navbar: React.FC = () => {
  ...
  const navItems = [
    { to: '/driver/dashboard', icon: Home, label: 'Home' },
    ...
    <>
      ...
      <nav className="hidden md:flex sticky top-0 z-50 w-full h-20 bg-background/70 backdrop-blur-2xl border-b border-border px-8">
        <div className="max-w-7xl mx-auto h-full flex items-center justify-between w-full relative">
          ...
          {/* Brand Logo */}
          <NavLink to="/driver/dashboard" className="flex items-center gap-3 group relative z-10">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[8px] font-black bg-accent/10 text-accent border border-accent/20 px-1.5 py-0.5 rounded uppercase tracking-widest">Driver</span>
              </div>
              <span className="text-[8px] font-bold text-muted uppercase tracking-[0.3em] mt-1">Fleet Command Node</span>
            </div>
          </NavLink>
          ...
          <div className="flex items-center gap-5 relative z-10">
            ...
            >
              ...
              <div className="w-10 h-10 rounded-full bg-surface border border-border flex items-center justify-center overflow-hidden hover:border-accent transition-all duration-300 group-active:scale-90 shadow-sm relative">
                 <span className="text-accent font-black text-sm relative z-10 group-hover:scale-110 transition-transform">
                   {(user as any)?.name?.charAt(0) || 'D'}
                 </span>
                 <div className="absolute bottom-1 right-1 w-2 h-2 bg-success rounded-full border-2 border-background z-20"></div>
              </div>
            </NavLink>
          </div>
        </div>
