      ...
      <div className="max-w-6xl mx-auto p-mobile-safe relative z-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
        <header className="mb-20 flex flex-col md:flex-row items-center justify-between gap-10 text-center md:text-left">
          ...
          <div className="flex items-center gap-4">
             ...
             <button className="btn-primary h-12 sm:h-16 px-6 sm:px-10 gap-3 sm:gap-4 text-[10px] sm:text-xs tracking-[0.2em]">
                <Plus className="w-4 h-4 sm:w-5 sm:h-5 leading-none" />
                PAYOUT
             </button>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-20">
          <div className="lg:col-span-8">
            <div className="glass-card p-6 sm:p-16 border-accent/10 shadow-2xl relative overflow-hidden group backdrop-blur-2xl">
               ...
               <div className="relative z-10 h-full flex flex-col justify-between min-h-[250px] sm:min-h-[350px]">
                 ...
                 <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-10">
                    <div className="space-y-6">
                       <h2 className="text-4xl sm:text-6xl md:text-9xl font-bold tracking-tighter text-primary leading-none group-hover:scale-[1.02] transition-transform duration-700">
                          ₹{balance.toLocaleString('en-IN')}<span className="text-xl sm:text-3xl md:text-6xl opacity-10">.00</span>
                       </h2>
          ...
          <div className="lg:col-span-4 h-full">
            <div className="glass-card p-6 sm:p-12 border-accent/10 shadow-2xl h-full flex flex-col justify-between relative overflow-hidden group backdrop-blur-2xl">
               ...
               <div>
                  ...
                  <h3 className="text-3xl sm:text-5xl font-bold text-primary mb-5 tracking-tighter uppercase leading-tight">
                ...
                <button 
                   ...
                   className="w-full mt-10 h-12 sm:h-14 rounded-xl bg-accent text-white font-bold uppercase tracking-[0.2em] text-[9px] sm:text-[10px] hover:bg-accent/90 transition-all shadow-2xl shadow-accent/40 flex items-center justify-center gap-4 active:scale-95"
                >
        ...
        <div className="mb-32">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-8 mb-12 px-6 relative">
             <div className="flex items-center gap-3 sm:gap-5">
                <h2 className="text-2xl sm:text-4xl font-bold tracking-tighter text-primary uppercase">Sync Log</h2>
                <div className="px-3 py-1 rounded-full bg-accent text-white text-[8px] sm:text-[9px] font-bold uppercase tracking-[0.3em] shadow-lg shadow-accent/20">
                   Real-time
                </div>
             </div>
          ...
          <div className="grid grid-cols-1 gap-6 px-4">
            {transactions.length > 0 ? (
              transactions.map((t) => (
                <div key={t.id} className="glass-card p-4 sm:p-8 flex items-center justify-between group hover:border-accent/30 transition-all relative overflow-hidden backdrop-blur-xl">
                  <div className="flex items-center gap-8 relative z-10">
                     <div className={`w-12 h-12 sm:w-20 sm:h-20 rounded-2xl flex items-center justify-center border shadow-2xl transition-all ${
                       t.type === 'credit' 
                       ? 'bg-accent/5 text-accent border-accent/20' 
                       : 'bg-rose-500/5 text-rose-500 border-rose-500/20'
                     }`}>
                       {t.type === 'credit' ? <ArrowDownLeft className="w-5 h-5 sm:w-10 sm:h-10" /> : <ArrowUpRight className="w-5 h-5 sm:w-10 sm:h-10" />}
                     </div>
                     <div className="min-w-0 space-y-2">
                        <p className="text-[9px] font-bold uppercase tracking-[0.4em] text-accent mb-1 opacity-50">{t.method}</p>
                        <h4 className="text-base sm:text-xl font-bold tracking-tight text-primary truncate leading-none uppercase">{t.title}</h4>
                  ...
                  <div className="text-right relative z-10">
                     <p className={`text-xl sm:text-5xl font-bold tracking-tighter leading-none ${t.type === 'credit' ? 'text-accent' : 'text-rose-500'}`}>
                       {t.type === 'credit' ? '+' : '−'} {t.amount}<span className="text-[10px] sm:text-2xl opacity-10">.00</span>
                     </p>
